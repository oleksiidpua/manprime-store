...
export default function Footer({ dict }: { dict: Dict }) {
  ...
  return (
    <footer className="bg-[#2d3748] border-t border-[#4a5568] mt-16">
      <div className="max-w-6xl mx-auto px-4 py-14 grid grid-cols-1 md:grid-cols-3 gap-10">
        ...
        <div>
          <div className="mb-4">
            ...
            <span className="block text-[#a0aec0] text-[9px] font-medium tracking-[0.35em] mt-0.5">STORE</span>
          </div>
          ...
          <div className="flex gap-3">
            {['facebook', 'instagram'].map((soc) => (
              <a
                key={soc}
                href="#"
                className="w-8 h-8 border border-[#4a5568] rounded flex items-center justify-center text-[#718096] hover:border-[#c9a84c] hover:text-[#c9a84c] transition-colors text-xs font-bold"
              >
                {soc === 'facebook' ? 'f' : 'ig'}
              </a>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={href}>
                <Link href={href} className="text-[#a0aec0] hover:text-[#c9a84c] text-sm transition-colors">
                  {label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-[#a0aec0] mb-8">
            <li>
            ...
            <li>{f.city}</li>
            <li className="text-[#718096]">{dict.delivery_info.text}</li>
          </ul>

          <p className="text-[#718096] text-xs mb-3 uppercase tracking-widest">Безпечна оплата</p>
          <div className="flex gap-2 flex-wrap">
            {['VISA', 'MC', 'LiqPay', 'Mono'].map((pay) => (
              <span
                key={pay}
                className="px-2 py-1 border border-[#4a5568] rounded text-[#718096] text-[10px] font-medium"
              >
                {pay}
              </span>
            ))}
      ...
      <div className="border-t border-[#4a5568] px-4 py-4 text-center text-[11px] text-[#718096]">
